...
{
    QString str = (QString::number(canMessage.numberSerialMessage, 10) + prefix + " ID_hdr:" + QString::number(canMessage.id_hdr, 16) +
                   " ID_body:" + QString::number(canMessage.id_body, 16) + " length: " + QString::number(canMessage.dataLen, 10) +
...
{
    QString str = ("--ansAD-- ID:" + QString::fromUtf8(answerArrayID.toHex(' ')) + " length: " + QString::number(lengthDataAnswer, 10) + " DATA:" + QString::fromUtf8(answerArrayDATA.toHex(' ')));
    return (str);
}
  ...
  //  qDebug() << "handleUartParsing ";
    ...
    for (int i=0; i<=(dataRead.size()-2); i++)
    {
        if ((quint8(dataRead.at(i)) == AD_COM_ID_FIRST_BYTE)) // если вероятен первый пакет
        {
            ...
            switch (quint8(dataRead.at(i+1))) { // проверяем второй бит после стартового
                case AD_COM_ID_CAN_1 :{  // это CAN сообщение
                    ...
                    else // если длина сообщения достаточная, заполняем поля ID, DATA и проверяем формат
                    {
                        ...
                        else
                        {
                            ...
                            QString extendedFrame = handleCAN(canMessage, EXT_PREFIX);
